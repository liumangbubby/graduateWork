...
<div class="main_div">
  ...
  <div class="right_div spe_div">
    ...
    <hr />
    <form id="form_1" method="post" action="/stuenroll/jsp/backyard/YearAction!delete.action">
    <table width="700" border="0" cellpadding="0" cellspacing="0" class="spe_table">
      ...
      <s:iterator id="obj" value="yearList" status="st">
      <tr style="background-color: ${st.index%2!=0?'#fcfcfc':'#E3FAE3'}">
      	<td height="32" align="center" class="cell">
        	<input name="yearId" id="yearId_${obj.id}" type="checkbox" value="${obj.id }" />
        </td>
        <td align="center" class="cell_num"><s:property value='#st.index+1'/></td>
        <td align="center" class="cell" name="yearName">${obj.year }</td>
        <td align="center" class="cell">
        <img src="../../pic/delete.png" width="22" onclick="deleteYear('${obj.id }')"/></td>
      </tr>
      </s:iterator>
      </s:if>
      <s:if test="yearList.size==0">
    ...
    <div class="spe_div">
      	...
      	<input type="button" value="删除" class="op_btn" onclick="deleteYear()"/>
      	<input type="button" value="添加" class="op_btn" onclick="addYear()"/>
     </div>
  </div>
</div>
</body>
